...
def encode(filepath):
    ...
    if video_codec[0] == 'hevc':
        ...
        video_opts = '-c:v libx265 -crf 29 -pix_fmt yuv420p -tag:v hvc15 -c:v 360k -metadata title=@anime_low_size encodes -preset medium -c:s copy -map 0'
    # Get the audio channel codec
    audio_codec = get_codec(filepath, channel='a:0')
    if audio_codec == []:
        audio_opts = ''
    elif audio_codec[0] == 'aac':
        audio_opts = '-c:a libvorbis -profile:a aac_he_v2 -vbr 2 -ab 60k -map 0:a? -map_metadata 0'
    else:
        audio_opts = '-c:a libopus -profile:a aac_he_v2 -vbr 2 -ab 60k -map 0:a? -map_metadata 0'
